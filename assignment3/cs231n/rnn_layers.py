...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    x_res = np.reshape(x,(x.shape[1],x.shape[0],x.shape[2]))
    T,N,D = x_res.shape
    H = h0.shape[1]
    h = np.zeros((N,T,H))
    cache = []
    ...
    for t in range(T):
      next_h, c = rnn_step_forward(x[:,t,:],h0,Wx,Wh,b)
      h[:,t,:] = next_h
      cache.append(c)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T):
      idx = (T-1)-t
      c = cache[idx]
      b = rnn_step_backward(dh[:,idx,:]+dh0,c)
...
def word_embedding_forward(x, W):
    ...
    N,T = x.shape
    out = np.array([[W[x[n,t]]for t in range(T)]for n in range(N)])
    cache = (x,W,out)
